...
const USER_EMAIL = 'test300401@yopmail.com';
...
(async () => {
  ...
  const transformStream = csv
    .transform(async function(data, done) {
      ...
      if (isEmpty(organization)) {
        console.log(`Importing ${data.siret} with domain ${emailDomain}...`);
      ...
      if (some(usersInOrganizationAlready, ['email', USER_EMAIL])) {
        console.log(
          `\x1B[31mWarning: ${USER_EMAIL} in organization already.\x1B[0m`
        );
      } else {
        console.log(
        ...
        await addUser({ organization_id: organization.id, user_id: user.id });
      }
